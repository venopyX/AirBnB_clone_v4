      ...
      for (const place of data) {
        const template = `<article>
          ...
          <div class="reviews">
            ...
            <ul class="review-list" data-place-id="${place.id}"></ul>
          </div>
        </article> <!-- End 1 PLACE Article -->`;
        $('section.places').append(template);
      }

      // Add event listener for toggling reviews
      $('.toggle-reviews').click(function () {
        const placeId = $(this).attr('data-place-id');
        const reviewList = $(`ul.review-list[data-place-id="${placeId}"]`);
        ...
        if (toggleText === 'show') {
          ...
          $.ajax({
            ...
            for (const review of reviews) {
              const reviewTemplate = `<li>${review.text} - <em>${review.user_id}</em></li>`;
